...
def lookup_mac(hostname):
    try:
        ...
        return None, None
    try:
        data=ssh.query(f'show ip arp {ip_address} vrf {vrf}', ssh_username=ssh_username, ssh_password=ssh_password, ssh_host=ssh_host)[0]
        log.info(f"found mac {data['mac']} for ip {ip_address} in vrf {vrf} on device {ssh_host}")
...
if mac_address:
    log.info(f"Adding {hostname.lower()} with mac {mac_address} to ise group id {endpoint_group}")
    result=cisco.ise.connect().add_endpoint(name=hostname.lower(), mac=mac_address.upper(), group_id=endpoint_group, description=description)
