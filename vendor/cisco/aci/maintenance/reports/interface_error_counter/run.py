...
for node_id in nodes_to_collect:
    ...
    for obj in cisco.aci.query(
        f"/api/node/mo/topology/pod-{pod_id}/node-{node_id}/sys.json?query-target=subtree&target-subtree-class=rmonDot3Stats&order-by=rmonDot3Stats.dn|desc"
    )["imdata"]:
