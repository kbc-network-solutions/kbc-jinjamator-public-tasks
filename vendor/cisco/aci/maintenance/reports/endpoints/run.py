...
lookup_table = {"epg": {}, "bd": {}}
epgs = cisco.aci.query(
    "/api/node/class/fvAEPg.json?rsp-subtree=children&order-by=fvAEPg.name"
)["imdata"]
bds = cisco.aci.query(
    "/api/node/class/fvBD.json?rsp-subtree=children&order-by=fvBD.name"
)["imdata"]
for result in bds:
    k = list(result.keys())[0]
    if k == "fvBD":
        bd = result[k]["attributes"]["dn"]
        lookup_table["bd"][bd] = {"name": result[k]["attributes"]["name"], "subnet": []}
...
# return lookup_table
data = {}
for line in cisco.aci.query(
    "/api/node/class/fvCEp.json?rsp-subtree=children&order-by=fvCEp.encap"
)["imdata"]:
